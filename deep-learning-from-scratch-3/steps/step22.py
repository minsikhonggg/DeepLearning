# step22 연산자 오버로드(3)
'''
- 부호 변환 연산자, 뺄셈, 나눗셈, 거듭제곱

- Function 클래스를 상속하여 원하는 함수 클래스 구현(예: Mul 클래스)
- 파이썬 함수로 사용할 수 있도록 만든다(예: mul 함수)
- Variable 클래스의 연산자를 오버로드 한다(예: Variable.__mul__ = mul)
'''
...
Variable.__rsub__ = rsub # 뺄셈의 경우 좌우항 이 바뀌면 값이 바뀐다
x = Variable(np.array(2.0))
y1 = 2.0 - x # x0 = x, x1 = 2.0 -> 2.0 - x
y2 = x - 1.0
...
y1 = 2.0 / x 
y2 = x / 1.0
print(y1) # variable(0.5)
print(y2) # variable(4.0)

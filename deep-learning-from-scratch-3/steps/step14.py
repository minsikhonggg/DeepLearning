...
y.backward()
print('y.grad: {}({})'.format(y.grad, id(y.grad))) # 1.0
print('x.grad: {}({})'.format(x.grad, id(x.grad))) # 2.0

y = add(add(x, x), x) # 같은 변수 재사용 -> 메모리 절약, 인스턴스 재사용
y.backward()
print(x.grad) # 2.0 + 3.0 = 5.0 -> 미분 값 초기화 메서드 필요

x.cleargrad() # 미분 값 초기화
y = add(add(x, x), x) 
y.backward()
print(x.grad) # 3.0